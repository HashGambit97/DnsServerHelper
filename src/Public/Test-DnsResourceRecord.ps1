function Test-DnsResourceRecord {
    <#
    .SYNOPSIS
        Tests for the existence of a DNS resource record across multiple DNS servers.
    .DESCRIPTION
        The Test-DnsResourceRecord cmdlet queries multiple DNS servers to check for the existence of a specified DNS resource record.
    .PARAMETER Name
        Specifies the name of the resource record to query for
    .PARAMETER New
        Switch to indicate whether to use the new set of DNS servers.
    .EXAMPLE
        Test-DnsResourceRecord -Name "example.com"
        This command queries the default set of DNS servers for the resource record "example.com".
    #>
    param (
