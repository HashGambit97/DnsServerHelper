function Set-CiscoDnsResourceRecord {
    ...
    Specifies a CIM session to use for this cmdlet.
    ...
    begin {
        ...
        if ($CimSession) {
            $CmdParam.Add('CimSession', $CimSession)
        }
    }
    ...
    process {
        ...
        foreach ( $Line in $RouterConfig ) {
            ...
            if ( $Line[1] -ne 'unassigned' -and $Line[3] -match 'dhcp|nvram|manual|tftp' ) {
                ...
                if ( $InterfaceName[0] -eq 'lo0' ) {
                    $InterfaceDns = "$($Name)"
                } elseif ( ( $InterfaceName[0] -eq 'fa0' -or $InterfaceName[0] -eq 'gi0/0' -or $InterfaceName[0] -eq 'gi0-0' ) -and !$MgmtInterface ) {
                ...
                if ($PSCmdlet.ShouldProcess($InterfaceDns, 'Update DNS resource record.')) {
                    Update-DnsServerResourceRecord -ZoneName $ZoneName -Name $InterfaceDns -IPAddress $IPAddress @CmdParam
                }
            }
        }
    }
